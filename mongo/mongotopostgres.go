...
func transferPosts(ctx context.Context, collection *mongo.Collection, pgPool *pgxpool.Pool) error {
	...
	for cursor.Next(ctx) {
		...
		_, err = pgPool.Exec(ctx, "INSERT INTO Post (id, author, title, content, coterie, createdAt, imageurl, hearts, comments) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)",
			post.ID, post.Author, post.Title, post.Content, post.Coterie, post.CreatedAt, post.ImageURL, pq.Array(post.Hearts), post.Comments)
		if err != nil {
			return err
		}
	}
